...
def find_path(G):
	...
	num_path = len(Queue)
	#print('queue', Queue)
	maxlen=verification(Queue,testG)
	return maxlen, num_path,Queue
...
def g_generator():
	...
	NUM_edge=inta[1]
	global edge_tot
	edge_tot=edge_tot+inta[0]*inta[0]
	#print(NUM_edge) 
	op = []
	network_matrix = np.zeros([NUM_NODES+1, NUM_NODES+1])
	for i in range(1,NUM_edge+1):
		network_matrix[inta[i*2]][inta[i*2+1]] = 1
		#network_matrix[inta[i*2+1]][inta[i*2]] = 1
...
if __name__ == '__main__':
	for i in range(1):
		...
		for _ in op:
			...
			maxlen, num_path,q= find_path(_)
			end = time.perf_counter()
			#print(q)
			print(num_path)
			#print(maxlen)
			#print(end - start)
#print((edge_tot+dfs_tot)*4)